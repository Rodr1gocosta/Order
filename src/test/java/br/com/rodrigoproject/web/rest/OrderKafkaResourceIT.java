...
import br.com.rodrigoproject.config.KafkaSseConsumer;
import br.com.rodrigoproject.config.KafkaSseProducer;
...
class OrderKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/order-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/order-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/order-kafka/unregister"));
                return;
            }
        }
